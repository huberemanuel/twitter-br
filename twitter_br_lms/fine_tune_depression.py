...
def load_and_prepare_dataset(dataset_name: str, model_name: str, do_preprocess: bool = True):
    ...
    return full_train_dataset, num_labels
...
def main():
    ...
    # covid19-fakenews
    # tt-depression
    dataset_name = "tt-depression"

    full_train_dataset, num_labels = load_and_prepare_dataset(dataset_name, selected_model)
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(full_train_dataset)):
        train_subsampler = torch.utils.data.SubsetRandomSampler(torch.tensor(train_idx))
        test_subsampler = torch.utils.data.SubsetRandomSampler(torch.tensor(test_idx))

        trainloader = DataLoader(full_train_dataset, batch_size=batch_size, sampler=train_subsampler)
        testloader = DataLoader(full_train_dataset, batch_size=batch_size, sampler=test_subsampler)
        ...
        model.to(device)
        best_train_acc, best_train_f1, best_test_acc, best_test_f1 = 0, 0, 0, 0
        ...
        for epoch in range(epochs):
            ...
            best_train_acc = max(best_train_acc, acc_score["accuracy"])
            best_train_f1 = max(best_train_f1, f_score["f1"])
            ...
            f_score = metric.compute(average="macro")
            acc_score = acc.compute()
            best_test_acc = max(best_train_acc, acc_score["accuracy"])
            best_test_f1 = max(best_train_f1, f_score["f1"])
        ...
        test_f1.append(best_test_f1)
    print("Training average acc: ", np.mean(train_acc))
    print("Training average f1: ", np.mean(train_f1))
    print("Validation average acc: ", np.mean(test_acc))
    print("Validation average f1: ", np.mean(test_f1))
    exit(0)
